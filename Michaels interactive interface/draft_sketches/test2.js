...
function draw() {
    ...
    if (detectedState > 0) {
        ...
        if (detectedState == 1) {
            background(canvas);
            //if it's at 1, clear the "intro" screen by changing the background to the canvas image, and begin painting.

            /* Also, since it's at 1, the  function gotPoses (see above) changes it to 2. This prevents the detectedState to go back to 0 (which draws a new background) and allows the brush strokes to draw over the background that was previously drawn at detectedState = 1.
            
            Basically, by switching dectedState from 1 or 2 when someone's present, prevents a background from being drawn at each frame, since no commands were given for detectedState = 2. So every time there's a new frame, it draws the brush strokes but not the background. The background you're seeing is the old one from when the detectedState was 1.*/
        }
